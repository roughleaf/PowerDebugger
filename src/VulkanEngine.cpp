...
VkInstance               VulkanEngine::g_Instance = VK_NULL_HANDLE;
VkPhysicalDevice         VulkanEngine::g_PhysicalDevice = VK_NULL_HANDLE;
VkDevice                 VulkanEngine::g_Device = VK_NULL_HANDLE;
uint32_t                 VulkanEngine::g_QueueFamily = (uint32_t)-1;
VkQueue                  VulkanEngine::g_Queue = VK_NULL_HANDLE;
VkDebugReportCallbackEXT VulkanEngine::g_DebugReport = VK_NULL_HANDLE;
VkPipelineCache          VulkanEngine::g_PipelineCache = VK_NULL_HANDLE;
VkDescriptorPool         VulkanEngine::g_DescriptorPool = VK_NULL_HANDLE;

ImGui_ImplVulkanH_Window VulkanEngine::g_MainWindowData;
int                      VulkanEngine::g_MinImageCount = 2;
bool                     VulkanEngine::g_SwapChainRebuild = false;
...
void VulkanEngine::check_vk_result(VkResult err)
{
    if (err == 0)
        return;
...
void VulkanEngine::SetupVulkan(const char** extensions, uint32_t extensions_count)
{
    VkResult err;
...
void VulkanEngine::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    wd->Surface = surface;
...
void VulkanEngine::CleanupVulkanWindow()
{
    ImGui_ImplVulkanH_DestroyWindow(g_Instance, g_Device, &g_MainWindowData, g_Allocator);
}

void VulkanEngine::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
    VkResult err;
...
void VulkanEngine::FramePresent(ImGui_ImplVulkanH_Window* wd)
{
    if (g_SwapChainRebuild)
        return;
    VkSemaphore render_complete_semaphore = wd->FrameSemaphores[wd->SemaphoreIndex].RenderCompleteSemaphore;
